    ...
    methods: {
      init (id) {
        ...
        this.$http({
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/Internetofthings/Equip/selectByPrimaryKey?equipid=${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                ...
                this.dataForm.equipid = data.content.equipid
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/Internetofthings/Equip/insertSelective`),
              method: 'post',
