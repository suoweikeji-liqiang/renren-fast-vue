<template>
  ...
  <div class="mod-gateway">

    <el-select v-model="value1" clearable placeholder="请选择楼宇">
      <el-option
    ...
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('sys:user:save')" type="primary" @click="addOrUpdateHandle()">添加网关</el-button>
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/Internetofthings/Gateway/showAllGateway'),
          method: 'get',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${hotelids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/Internetofthings/Gateway/deleteByGateway'),
            method: 'post',
...
//导出
      exportHandle(){
          var pash=""
          this.$http({
            url: this.$http.adornUrl('/Internetofthings/Gateway/exportSupplier'),
            method: 'post',
            data: this.$http.adornData({
                'hotelid': this.dataForm.id || undefined,
                'onlinestatus': this.dataForm.onlinestatus,
                'refreshtime': this.dataForm.refreshtime,
                'hotelcode': this.dataForm.hotelcode,
