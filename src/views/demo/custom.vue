<template>
  ...
  <div class="mod-custom">
    ...
    <el-select v-model="value1" clearable placeholder="请选择客户名称">
      <el-option
    ...
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('sys:user:save')" type="primary" @click="addOrUpdateHandle()">添加客户</el-button>
...
<script>
  import AddOrUpdate from './custom-add-or-update'
  export default {
    data () {
      return {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/Internetofthings/Customer/showAllSupplier'),
          method: 'get',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${hotelids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/Internetofthings/Customer/deleteBysupplier'),
            method: 'post',
...
//导出
      exportHandle(){
          var pash=""
          this.$http({
            url: this.$http.adornUrl('/Internetofthings/Customer/exportSupplier'),
            method: 'post',
